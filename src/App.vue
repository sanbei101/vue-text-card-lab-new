...
const keyword = computed(() => {
  if (!autoKeyword.value) return customKeyword.value;
...
watch(
  title,
  () => {
    if (autoKeyword.value) customKeyword.value = inferKeyword(title.value);
  },
  {
    immediate: true,
  },
...
async function exportTemplate(template: CardTemplate) {
  ...
  const svg = document.querySelector("#export-stage svg") as SVGSVGElement | null;

  if (!svg) return;

  await svgToPng(svg, `text-card-${template.id}.png`);
}
  ...
  <main class="mx-auto min-h-screen max-w-5xl min-w-[320px] pb-20 font-sans text-zinc-900">
    ...
    <section
      ...
      <p class="m-0 mb-1.25 max-w-127.5 leading-[1.7] text-zinc-500">
        不是简单换背景色：每个模板都有自己的文字区域、 对齐方式、装饰系统、最大行数和字号范围。
      </p>
    </section>
