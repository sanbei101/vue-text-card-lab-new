...
const textAnchor = computed(() => {
  if (props.template.textBox.align === 'center')
    return 'middle'

  if (props.template.textBox.align === 'right')
    return 'end'
...
const particles = computed(() =>
  Array.from({ length: 14 }, (_, index) => ({
    ...
    y:
      90
      + seededValue(seed.value, index + 20)
        * 1020,
    size:
      8
      + seededValue(seed.value, index + 40)
        * 22,
    rotation:
      seededValue(seed.value, index + 60)
        * 180,
  })),
)
</script>
  ...
  >
    <defs>
      ...
      >
        <stop
          offset="0"
          :stop-color="template.background"
        />
        <stop
          offset="1"
          :stop-color="template.muted"
          stop-opacity="0.36"
        />
      </linearGradient>
    ...
    <rect
      ...
      :rx="template.radius"
      :fill="
        template.kind === 'lavender'
          ? `url(#gradient-${template.id})`
          : template.background
      "
    ...
    >
      <text
        ...
        y="260"
        font-size="310"
        transform="rotate(-18 -80 260)"
      >?</text>
      <text
        x="300"
        y="390"
        font-size="430"
        transform="rotate(32 300 390)"
      >?</text>
      <text
        x="560"
        y="1040"
        font-size="330"
        transform="rotate(-20 560 1040)"
      >?</text>
    ...
    <g v-if="template.kind === 'stamp'">
      ...
      />
      <g
        transform="translate(690 945) rotate(-12)"
        opacity="0.8"
      >
        <rect
          width="125"
    ...
    <g
      v-if="
        !['memo', 'editorial', 'paper', 'mono'].includes(
          template.kind,
        )
      "
      :fill="template.foreground"
