...
function linePenalty(
  ...
  // 非末行更强调视觉均衡，末行允许短一些。
  penalty += isLast
    ? Math.pow(Math.max(0, 0.72 - fill), 2) * 520
    : Math.pow(1 - fill, 2) * 820
  ...
  if (BREAK_AFTER.has(last))
    penalty -= 100

  if ([...line].length <= 2)
    penalty += isLast ? 680 : 1100
...
export function buildTextLayout(
  ...
  const lineHeightPx =
    bestSize * template.textBox.lineHeight
...
export function findHighlightRects(
  ...
  layout.lines.forEach((line, lineIndex) => {
    ...
    while (from < line.length) {
      ...
      const beforeWidth = measureText(
        before,
        layout.fontSize,
        template,
      )
      const matchedWidth = measureText(
        matched,
        layout.fontSize,
        template,
      )
      ...
      rectangles.push({
        ...
        y:
          box.y
          + lineIndex * layout.lineHeightPx
          + layout.fontSize * 0.55,
...
export function seededValue(seed: number, index: number) {
  const x = Math.sin(seed * 12.9898 + index * 78.233)
    * 43758.5453
